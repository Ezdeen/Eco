...
async function main() {
  ...
  await db.userMembership.createMany({
    data: [
      ...
      { userId: esgManager.id, organizationId: org.id, role: 'org_admin', status: 'active' },
      { userId: operator.id, organizationId: org.id, role: 'project_manager', status: 'active' },
      { userId: viewer.id, organizationId: org.id, role: 'project_manager', status: 'active' },
  ...
  console.log('\n📋 Login Credentials:')
  console.log('   ────────────────────────────────────────')
  console.log('   Admin:              admin@bfec.sa / Admin@123456        (org_admin)')
  console.log('   ESG Manager:        esg@bfec.sa / ESG@123456           (org_admin)')
  console.log('   Project Manager:    project@bfec.sa / Project@123456    (project_manager)')
  console.log('   Operator:           operator@bfec.sa / Operator@123456  (project_manager)')
  console.log('   Viewer:             viewer@bfec.sa / Viewer@123456      (project_manager)')
}
