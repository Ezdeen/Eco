...
const ECO_STYLES = `
.eco-auth {
  --eco-line: rgba(210, 245, 235, 0.13);
  --eco-border: rgba(206, 246, 234, 0.28);
  --eco-border-strong: rgba(150, 246, 218, 0.55);
  --eco-fg: #eef8f4;
  --eco-muted: #cfe1db;
  --eco-emerald: #5eeaa9;
  --eco-teal: #55e6d3;
  --eco-cyan: #55dcf0;
  --eco-panel: rgba(38, 88, 82, 0.62);
  position: relative;
  display: flex;
  min-height: 100vh;
  align-items: center;
  ...
  background: radial-gradient(120% 85% at 84% 4%, #4aa88d 0%, #378a78 40%, #2a7268 74%, #236460 100%);
  color: var(--eco-fg);
...
.eco-auth .eco-blob { position: absolute; border-radius: 9999px; filter: blur(90px); opacity: 0.42; }
.eco-auth .eco-blob-a {
...
.eco-auth .eco-canvas { position: absolute; inset: 0; height: 100%; width: 100%; opacity: 0.5; }
.eco-auth .eco-scan {
...
.eco-auth .eco-vignette {
  ...
  background: radial-gradient(105% 85% at 50% 45%, rgba(30, 86, 82, 0) 58%, rgba(30, 86, 82, 0.24) 100%);
}
.eco-auth .eco-shell {
  ...
  box-shadow:
    ...
    0 40px 90px -44px rgba(2, 16, 18, 0.7),
    0 0 0 1px rgba(6, 30, 28, 0.35);
...
.eco-auth .eco-hero {
  ...
  background:
    radial-gradient(115% 90% at 12% 0%, rgba(92, 194, 166, 0.92) 0%, rgba(56, 148, 130, 0.94) 58%, rgba(40, 116, 110, 1) 100%);
}
@media (min-width: 900px) { .eco-auth .eco-hero { display: flex; min-height: 620px; } }
@media (min-width: 1024px) { .eco-auth .eco-hero { min-height: 660px; } }
...
.eco-auth .eco-ring {
  position: absolute; border-radius: 9999px; border: 1px solid rgba(168, 234, 217, 0.2);
}
...
.eco-auth .eco-track { height: 6px; border-radius: 9999px; background: rgba(232, 252, 246, 0.34); overflow: hidden; }
.eco-auth .eco-fill {
...
.eco-auth .eco-trust-item { display: flex; align-items: center; gap: 0.5rem; font-size: 0.72rem; letter-spacing: 0.02em; color: rgba(244, 252, 249, 0.86); }
.eco-auth .eco-trust-sep { height: 1rem; width: 1px; background: rgba(190, 240, 228, 0.34); }
.eco-auth .eco-form-side { position: relative; display: flex; flex-direction: column; justify-content: center; padding: 2.5rem 1.5rem; background: linear-gradient(180deg, rgba(44, 104, 95, 0.6), rgba(34, 88, 84, 0.74)); }
...
.eco-auth .eco-lang {
  ...
  border: 1px solid rgba(206, 248, 236, 0.34) !important;
  background: rgba(44, 108, 98, 0.72) !important;
...
.eco-auth .eco-lang:hover { border-color: var(--eco-border-strong) !important; background: rgba(60, 134, 120, 0.9) !important; transform: translateY(-1px); }
.eco-auth .eco-field {
  height: 3rem !important; border-radius: 14px !important;
  border: 1px solid rgba(206, 248, 236, 0.36) !important;
  background: rgba(34, 84, 80, 0.62) !important;
  color: var(--eco-fg) !important;
  caret-color: var(--eco-teal);
...
.eco-auth .eco-field::placeholder { color: rgba(212, 236, 229, 0.88); }
.eco-auth .eco-field:hover { border-color: rgba(200, 246, 233, 0.46) !important; }
.eco-auth .eco-field:focus-visible {
  border-color: var(--eco-border-strong) !important;
  background: rgba(38, 92, 86, 0.8) !important;
...
.eco-auth .eco-demo {
  ...
  border-radius: 14px; border: 1px solid rgba(206, 248, 236, 0.32);
  background: rgba(34, 84, 80, 0.52); padding: 0.7rem;
  text-align: start;
...
.eco-auth .eco-demo:hover { border-color: var(--eco-border-strong); background: rgba(48, 116, 106, 0.78); transform: translateY(-1px); }
.eco-auth .eco-demo:active { transform: translateY(0) scale(0.993); }
.eco-auth .eco-link {
  color: #86e3cf; text-decoration: underline; text-underline-offset: 4px; text-decoration-thickness: 1px;
  transition: color 200ms cubic-bezier(0.25, 1, 0.5, 1);
}
.eco-auth .eco-link:hover { color: #bbf7e0; }
...
export function LoginSection({ onLoginSuccess, locale, onLocaleChange }: LoginSectionProps) {
  ...
  return (
    ...
    >
        ...
        <section className="eco-hero" aria-label="Eco Ledger">
          ...
          <div className="relative flex items-center gap-3">
            ...
            <div>
              ...
              <p className="mt-0.5 text-xs text-emerald-50/75">{loginCopy[locale].platformDescription}</p>
            </div>
          </div>
          ...
          <div className="relative my-auto max-w-md pt-14">
            ...
            <p className="mt-5 max-w-prose text-[0.95rem] leading-7 text-emerald-50/85">
              ...
              {TELEMETRY.map((row, index) => (
                <div key={row.label} className={`eco-in eco-in-${index + 2}`}>
                  <div className="mb-2 flex items-center justify-between gap-3">
                    ...
                    <span className="text-[0.7rem] font-medium text-emerald-50/80">{loginCopy[locale][row.sub]}</span>
                  </div>
          ...
          <div className="eco-in mx-auto w-full max-w-md">
            ...
            <Card className="border-0 bg-transparent shadow-none">
              <CardHeader className="space-y-2 p-0">
                ...
                <CardDescription className="max-w-sm text-sm leading-6 text-emerald-50/80">
              ...
              <CardContent className="p-0 pt-7">
                <form onSubmit={handleSubmit} className="space-y-5" noValidate>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <button
                        type="button"
                        className="eco-link rounded text-xs font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400/50 focus-visible:ring-offset-2 focus-visible:ring-offset-[#16403a]"
                ...
                <div className="relative my-7">
                  ...
                  <span className="absolute start-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-[#16403a] px-3 text-xs text-emerald-100/70">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {DEMO_ACCOUNTS.map((account) => (
                    <button
                      ...
                      className={`eco-demo focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-[#16403a] ${account.ring}`}
                    >
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        <span className="mt-1 block truncate text-[11px] text-emerald-50/75" dir="ltr">{account.email}</span>
                      </span>
                    </button>
                  ))}
                </div>

                <p className="mt-4 flex items-center justify-center gap-1.5 text-center text-xs leading-5 text-emerald-100/60">
      ...
      <p className="absolute bottom-4 z-10 text-center text-xs text-emerald-50/80">
