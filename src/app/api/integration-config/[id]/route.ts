...
export async function PATCH(request: NextRequest, { params }: Params) {
  try {
    ...
    // Guard against a known footgun: Hedera silently defaulting to "simulation" mode
    // when the network field is left blank. Determine the effective isActive/config
    // after this update, and require a valid network if Hedera would end up active.
    const effectiveIsActive = isActive !== undefined ? isActive : existing.isActive
    if (existing.name === 'hedera' && effectiveIsActive) {
      const effectiveConfig = config !== undefined ? config : (existing.config ? JSON.parse(existing.config) : {})
      const network = effectiveConfig?.network
      if (!network || !['simulation', 'testnet', 'mainnet'].includes(network)) {
        return NextResponse.json(
          { error: 'يجب تحديد "الشبكة" (testnet أو mainnet) قبل تفعيل تكامل Hedera' },
          { status: 400 },
        )
      }
    }
